...
import com.lianwei.store.utils.PaymentUtil;
...
public class OrderServlet extends BaseServlet {
		...
		public String saveOrder(HttpServletRequest req, HttpServletResponse resp)  throws Exception{
			...
			for (CartItem cartItem : cart.getCartItems()) {
				...
				orderItem.setTotal(cartItem.getTotalNum());
		...
		public String findOrderWithNumAndUser(HttpServletRequest req, HttpServletResponse resp)  throws Exception{
			//获取登录用户和页数
			User user = (User)req.getSession().getAttribute("loginUser");
			int num = Integer.parseInt(req.getParameter("num"));
			//根据用户和页数查询出对应的订单信息，保存到分页pageModel中
			OrderService orderService = new OrderServiceImpl();
			PageModel pageModel = orderService.findOrderWithNumAndUser(user,num);
			//将对应的pageModel用request存储起来，返回到页面
			req.setAttribute("page", pageModel);
			//返回到/jsp/order_list.jsp页面
		...
		public String payOrder(HttpServletRequest req, HttpServletResponse resp)  throws Exception{
			...
			String p7_Pdesc = "";
			//接受响应参数的Servlet
			String p8_Url = "http://localhost:48080/store/OrderServlet?method=callBack";
			String p9_SAF = "";
			String pa_MP = "";
