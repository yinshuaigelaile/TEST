...
public class PageModel {
	...
	private int totalRecords;//总记录条数，数据库查出来的			   * select count(*) from book where cid = ? 
	private int totalPageNum;//总页数，计算出来的		*totalPageNum=totalRecords%pageSize==0?totalRecords/pageSize:(totalRecords/pageSize+1)
	private int startIndex;//每页开始记录的索引，计算出来的			    *startIndex=(currentPageNum-1)*pageSize   
	private int prePageNum;//上一页							    *prePageNum = currentPageNum-1
	private int nextPageNum;//下一页							    *nextPageNum = currentPageNum-1
	
	private List records;//已经分好页的结果集,该list中只有10条记录   select * from user limit (currentPage-1)*pageSize,pageSize    
